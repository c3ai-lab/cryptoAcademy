...
export default {
  ...
  computed: {
    returnsSeries() {
      if (this.isLoaded) {
        return [{
          ...
          data: this.selectedReturnsData.map(
            entry => [
              entry.made_at,
    ...
    selectedReturnsData() {
      if (this.selectedReturnsSymbol.name == this.getAllReturnsOption().value.name) {
        return this.$store.getters["transactions/getReturns"]()
      } else {
        return this.$store.getters["transactions/getTransactionsBySymbol"](this.selectedReturnsSymbol.api_symbol)
      }
    },
  ...
  async created() {
    ...
    for (const idx in this.returnsSymbols) {
      await this.$store.dispatch("transactions/fetchReturnsPerSymbol", {symbol: this.returnsSymbols[idx]});
    }
  ...
  methods: {
    getAllReturnsOption() {
      return {value: { name: this.$t('common.all') }, text: this.$t('common.all')}
    },

    getOptions() {
      return [
          this.getAllReturnsOption(),
          ...this.returnsSymbols.map(
            symbol => ({ value: symbol, text: symbol.name })
          )
      ];
    },
  },
